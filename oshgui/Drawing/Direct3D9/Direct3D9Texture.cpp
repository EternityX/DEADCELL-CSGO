...
namespace OSHGui {
	namespace Drawing {
		...
		void Direct3D9Texture::LoadFromMemory( const void *buffer, const SizeF &size, PixelFormat pixelFormat ) {
			...
			auto hr = D3DXLoadSurfaceFromMemory( surface, nullptr, nullptr, pixelBuffer.GetPixelData(),
			                                     d3dFormat == D3DFMT_X8R8G8B8 ? D3DFMT_R8G8B8 : d3dFormat,
			                                     pixelBuffer.GetPitch(), nullptr, &rect, D3DX_FILTER_NONE, 0 );
		...
		void Direct3D9Texture::CreateDirect3D9Texture( const SizeF &size, D3DFORMAT format ) {
			...
			if( FAILED(D3DXCreateTexture(owner.GetDevice(), static_cast<UINT>(textureSize.Width), static_cast<UINT>(textureSize.
				Height), 1, 0, format, D3DPOOL_MANAGED, &texture)) ) {
				throw Misc::Exception();
			}
		...
		void Direct3D9Texture::PostD3DReset() {
			...
			owner.GetDevice()->CreateTexture( savedSurfaceDescription.Width, savedSurfaceDescription.Height, 1,
			                                  ...
			                                  savedSurfaceDescription.Pool, &texture, 0 );
