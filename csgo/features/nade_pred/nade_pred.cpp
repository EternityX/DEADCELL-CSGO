	...
	if (g_cl.m_local && g_cl.m_local->health() > 0 && ((m_type) && m_path.size() > 1)) {
		...
		for (auto it = m_path.begin(), end = m_path.end(); it != end; ++it) {
			if (g_visuals.world_to_screen(prev, nade_start) && g_visuals.world_to_screen(*it, nade_end)) {
				g_renderer.line(OSHColor::FromRGB(150, 150, 150), (int)nade_start.x, (int)nade_start.y, (int)nade_end.x, (int)nade_end.y);
			} prev = *it;
		}

		for (auto it = m_others.begin(), end = m_others.end(); it != end; ++it) {
			if (g_visuals.world_to_screen(it->first, collision_end)) {
				g_renderer.filled_rect(OSHColor::FromRGB(206, 115, 136), (int)collision_end.x - 1, (int)collision_end.y - 1, 3, 3);
				g_renderer.rect(OSHColor::FromRGB(0, 0, 0), (int)collision_end.x - 2, (int)collision_end.y - 2, 4, 4);
			}
		}

		if (g_visuals.world_to_screen(prev, nade_end)) {
			g_renderer.filled_rect(OSHColor::FromRGB(255, 255, 255), (int)nade_end.x - 1, (int)nade_end.y - 1, 3, 3);
			g_renderer.rect(OSHColor::FromRGB(0, 0, 0), (int)nade_end.x - 2, (int)nade_end.y - 2, 4, 4);
		}
	}
}
