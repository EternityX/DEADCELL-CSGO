...
int c_engine_pred::post_think( c_base_player *player ) const {
	static auto PostThinkVPhysics = pattern::find< bool( __thiscall*)( c_base_entity * ) >( g_csgo.m_client_dll, "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 8B D9 56 57 83 BB ? ? ? ? ? 0F 84" );
...
void c_engine_pred::start( c_user_cmd *ucmd ) {
	...
	//	CPrediction::StartCommand
	{
		...
		*reinterpret_cast< c_user_cmd ** >( uint32_t( player ) + 0x3338 ) = ucmd; // m_pCurrentCommand
