...
struct ImColor
{
    ...
    ImColor(const int* col)                                         { float sc = 1.0f/255.0f; Value.x = (float)col[1] * sc; Value.y = (float)col[2] * sc; Value.z = (float)col[3] * sc; Value.w = (float)col[0] * sc; } // dangerous
    ImColor(const int* col, int a)                                  { float sc = 1.0f/255.0f; Value.x = (float)col[1] * sc; Value.y = (float)col[2] * sc; Value.z = (float)col[3] * sc; Value.w = (float)a * sc; } // dangerous
    ImColor(const float* col)                                       { Value.x = col[1]; Value.y = col[2]; Value.z = col[3]; Value.w = col[0]; } // dangerous
    ImColor(const float* col, float a)                              { Value.x = col[1]; Value.y = col[2]; Value.z = col[3]; Value.w = a; } // dangerous
...
struct ImColorARGB
{
    ...
    ImColorARGB()                                                   { Value.x = Value.y = Value.z = Value.w = 0.0f; }
    ImColorARGB(int a, int r, int g, int b)                         { float sc = 1.0f/255.0f; Value.x = (float)r * sc; Value.y = (float)g * sc; Value.z = (float)b * sc; Value.w = (float)a * sc; }
    ImColorARGB(ImU32 rgba)                                         { float sc = 1.0f/255.0f; Value.x = (float)((rgba>>IM_COL32_R_SHIFT)&0xFF) * sc; Value.y = (float)((rgba>>IM_COL32_G_SHIFT)&0xFF) * sc; Value.z = (float)((rgba>>IM_COL32_B_SHIFT)&0xFF) * sc; Value.w = (float)((rgba>>IM_COL32_A_SHIFT)&0xFF) * sc; }
    ImColorARGB(float a, float r, float g, float b)                 { Value.x = r; Value.y = g; Value.z = b; Value.w = a; }
    ImColorARGB(const ImVec4& col)                                  { Value = col; }
    ImColorARGB(const int* col)                                     { float sc = 1.0f/255.0f; Value.x = (float)col[1] * sc; Value.y = (float)col[2] * sc; Value.z = (float)col[3] * sc; Value.w = (float)col[0] * sc; } // dangerous
    ImColorARGB(const int* col, int a)                              { float sc = 1.0f/255.0f; Value.x = (float)col[1] * sc; Value.y = (float)col[2] * sc; Value.z = (float)col[3] * sc; Value.w = (float)a * sc; } // dangerous
    ImColorARGB(const float* col)                                   { Value.x = col[1]; Value.y = col[2]; Value.z = col[3]; Value.w = col[0]; } // dangerous
    ImColorARGB(const float* col, float a)                          { Value.x = col[1]; Value.y = col[2]; Value.z = col[3]; Value.w = a; } // dangerous
    inline operator ImU32() const                                   { return ImGui::ColorConvertFloat4ToU32(Value); }
    inline operator ImVec4() const                                  { return Value; }

    // FIXME-OBSOLETE: May need to obsolete/cleanup those helpers.
    inline void        SetHSV(float h, float s, float v, float a = 1.0f){ ImGui::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z); Value.w = a; }
    static ImColorARGB HSV(float h, float s, float v, float a = 1.0f)   { float r,g,b; ImGui::ColorConvertHSVtoRGB(h, s, v, r, g, b); return ImColorARGB(a,r,g,b); }
};
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontAwesome( float size_pixels, const ImFontConfig* font_cfg = NULL );
